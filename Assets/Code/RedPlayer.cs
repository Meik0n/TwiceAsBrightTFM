...
public class RedPlayer : MonoBehaviour
{
    ...
    {
        if (Input.GetKeyDown(KeyCode.F) && flagSimple)
        {
    ...
    {
        WallManager.Instance.ChangeSimpleRedWalls();
    }

    private void CrossWalls()
    {
        if (Input.GetKeyDown(KeyCode.C) && flagCross)
        {
    ...
    {
        WallManager.Instance.ChangeCrossRedWalls();
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            case "RedEnd":
                photonView.RPC("ActivateChangeRedEndWall", RpcTarget.AllViaServer);
                Destroy(WallManager.Instance.RedEndFlame);
                break;
            case "Ending":
                WallManager.Instance.RedOnEndPlatform = true;
                break;
            case "Enemy":
    ...
    [PunRPC]
    void ActivateChangeRedEndWall()
    {
        WallManager.Instance.ChangeRedEndWall();
    }
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        {
            case "Ending":
                WallManager.Instance.RedOnEndPlatform = false;
                break;
        }
    }
